...
class SegmentationTrainer(monai.engines.SupervisedTrainer):
    ...
    def __init__(
        ...
        self.distributed = (
            hasattr(config, "distributed")
            and config.distributed
            and rank is not None
            and world_size is not None
        )

        # Only rank 0 (or non-distributed) should prepare directories
        if not self.distributed or self.rank == 0:
            self._prepare_dirs()

        # Synchronize processes after directory creation if distributed
        if self.distributed:
            dist.barrier()

        # Set device based on rank if distributed, otherwise use config
        if self.distributed:
            self.config.device = torch.device(f"cuda:{self.rank}")
        else:
            self.config.device = torch.device(self.config.device)

        # Create dataloaders with distributed support if needed
        if self.distributed:
        ...
        network = get_model(config=config).to(self.config.device)
        ...
        # Only add validation handlers on main process in distributed mode
        if not self.distributed or self.rank == 0:
            val_handlers = get_val_handlers(
                network.module if self.distributed else network, config=config
            )
        else:
            val_handlers = []
        ...
        # Only add monitoring for non-distributed or rank 0
        if not self.distributed or self.rank == 0:
            if early_stopping:
    ...
    def load_checkpoint(self, checkpoint=None):
        ...
        # Load state dict - handle DDP case by accessing .module if needed
        if self.distributed:
            self.network.module.load_state_dict(
                torch.load(checkpoint, map_location=self.config.device)
            )
        else:
            self.network.load_state_dict(torch.load(checkpoint, weights_only=False))
    ...
    def run(self, try_resume_from_checkpoint=True) -> None:
        ...
        # Only try to resume on main process to avoid file access conflicts
        if try_resume_from_checkpoint and (not self.distributed or self.rank == 0):
            checkpoints = [
            ...
            try:
                ...
                print(
                    f"{'Rank ' + str(self.rank) + ': ' if self.distributed else ''}resuming from previous checkpoint at {checkpoint}"
                )
            except:
                pass  # train from scratch

        # Synchronize all processes after checkpoint loading if distributed
        if self.distributed:
            dist.barrier()
        ...
        # Only process metrics on main process
        if not self.distributed or self.rank == 0:
            ...
            self.loss = {
                ...
                "epoch": [
                    _iter // self.state.epoch_length
                    for _iter, _ in self.metric_logger.loss
                ],
            }
    ...
    def evaluate(self, checkpoint=None, dataloader=None):
        ...
        # Skip evaluation on non-main processes
