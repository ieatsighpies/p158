...
def inference_pipeline(
    ...
    """
    # First, get the original image dimensions from t2
    t2_img = nib.load(t2_path)
    original_shape = t2_img.get_fdata().shape
    print(f"Original raw image shape: {original_shape}")

    # Also show other image shapes
    print("Original raw image shapes:")
    ...
    print(f"After preprocessing shapes:")
    ...
    with torch.no_grad():
        ...
        print(f"Thresholded prediction shape: {tumor_pred.shape}")

        # Resize prediction back to original raw dimensions using interpolate
        final_pred = F.interpolate(tumor_pred, size=original_shape, mode="nearest")
        print(f"Final prediction shape (matching raw input): {final_pred.shape}")

    # Save output
    print(f"Saving output to: {output_path}")
    final_pred = final_pred.squeeze().cpu()  # Remove batch and channel dims for saving
    saver = SaveImage(
