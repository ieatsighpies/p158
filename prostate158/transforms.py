...
def get_base_transforms(config: dict, minv: int = 0, maxv: int = 1) -> list:
    ...
    # Scale intensity and normalize
    tfms += [ScaleIntensityd(keys=config.data.image_cols, minv=minv, maxv=maxv)]
...
def get_train_transforms(config: dict):
    ...
    if "rand_crop_pos_neg_label" in config.transforms.keys():
        ...
        raise ValueError(
            "Either `rand_crop_pos_neg_label` or `rand_spatial_crop_samples` "
